...
fn sdb_slice_to_vec(s: &slice) -> Vec<u8> {
    unsafe { std::slice::from_raw_parts(s.data as *const u8, s.length as usize).to_vec() }
}

fn raw_to_vec(data: *const ::std::os::raw::c_void, length: usize) -> Vec<u8> {
    unsafe { std::slice::from_raw_parts(data as *const u8, length as usize).to_vec() }
}
...
fn sdb_msg_from_acc(ma: &merge_accumulator) -> SdbMessage {
    unsafe {
        SdbMessage {
            msg_type: int_to_msg_type(crate::raw::merge_accumulator_message_class(ma)),
            data: sdb_slice_to_vec(&crate::raw::merge_accumulator_to_slice(ma)),
        }
    }
}
...
pub extern "C" fn merge_tuples<T: SdbRustDataFuncs>(
    ...
    let new_msg: SdbMessage = unsafe { sdb_msg_from_acc(&*new_message) };
    ...
    unsafe {
        crate::raw::merge_accumulator_copy_message(
            new_message,
            message {
...
pub extern "C" fn merge_tuples_final<T: SdbRustDataFuncs>(
    ...
    let new_msg: SdbMessage = unsafe { sdb_msg_from_acc(&*oldest_message) };

    // call user's merge_final() function
    let res: SdbMessage = match T::merge_final(&sdb_slice_to_vec(&key), new_msg) {
        Ok(r) => r,
        Err(..) => return -1,
    };

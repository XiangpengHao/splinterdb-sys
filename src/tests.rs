...
mod tests {
    ...
    fn many_ins_lookup() -> Result<()> {
        ...
        for i in 0..=100 {
            let key = ("some-key-".to_owned() + &i.to_string())
                .into_bytes()
                .to_vec();
            let value = ("some-value-".to_owned() + &i.to_string())
                .into_bytes()
                .to_vec();
            sdb.insert(&key, &value)?;
        }
    ...
    impl SdbRustDataFuncs for SimpleMerge {
        ...
        fn merge(_key: &[u8], old_msg: SdbMessage, new_msg: SdbMessage) -> Result<SdbMessage> {
            ...
            let upd_val = if old_val >= new_val { old_val } else { new_val };
    ...
    fn simple_merge_test() -> Result<()> {
        ...
        sdb.update(
            &(b"some-key-6".to_vec()),
            &(b"some-value-9999999999999999999".to_vec()),
        )?;
